...
// Lightweight inline icon fallbacks to avoid external dependency in this example
const AlertCircle = (props: any) => <span {...props}>⚠️</span>;
const AlertTriangle = (props: any) => <span {...props}>△</span>;
const Info = (props: any) => <span {...props}>i</span>;
const XCircle = (props: any) => <span {...props}>✖</span>;
const RefreshCw = (props: any) => <span {...props}>⟳</span>;
import { useState } from "react";
import { getErrorMessage, getErrorSeverityClass } from "../utils/utils";
import type { ErrorDisplayProps } from "../types/types";
import { ErrorSeverity } from "../types/types";
...
export function ErrorDisplay({
  ...
  return (
    ...
    >
      <div
        className={`max-w-md w-full rounded-lg border p-6 shadow-lg ${severityClass}`}
      >
        <div className="flex items-start gap-3">
          {showIcon && (
            <Icon className="h-6 w-6 flex-shrink-0 mt-0.5" aria-hidden="true" />
          )}
