...
import type { AppError, ErrorHandlerConfig } from "../types/types";
...
class ErrorHandlerService {
  ...
  handle(error: AppError): void {
    ...
    if (
      this.config.reportToService &&
      error.severity === ErrorSeverity.CRITICAL
    ) {
      this.config.reportToService(error).catch((reportError: unknown) => {
  ...
  private handleGlobalError = (event: ErrorEvent): void => {
    event.preventDefault();
    const appError = this.categorizeError(
  ...
  private logError(error: AppError): void {
    const logMethod =
      error.severity === ErrorSeverity.CRITICAL ? "error" : "warn";
